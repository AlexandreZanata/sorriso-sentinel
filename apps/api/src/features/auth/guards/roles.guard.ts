...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const request = context.switchToHttp().getRequest<{ session?: SessionClaims }>();
    const sessionRoles = request.session?.roles ?? [];

    const allowed = requiredRoles.some((role) => sessionRoles.includes(role));
